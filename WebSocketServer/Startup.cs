        ...
        private async Task Send(HttpContext context, WebSocket webSock)
        {
            ...
            if(result != null)
            {
                while(!result.CloseStatus.HasValue)
                {
                    ...
                    await webSock.SendAsync(new ArraySegment<byte>(Encoding.UTF8.GetBytes($"Server says:{DateTime.UtcNow:f} ")), result.MessageType, result.EndOfMessage, System.Threading.CancellationToken.None);
